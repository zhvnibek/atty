...
def add_course_session(session, course_id: int, date: str):
    course_date_id = None
    select_query = \
    f"""SELECT id
        FROM course_date
...
def check_student_at_course_session(session, stud_label: str, course_id: str, date: str) -> bool:
    """Update student_course_date table"""
    logger.info(f"label={stud_label}, course_id={course_id}, date={date}")
    stud_id: int = get_id(session, stud_label)
    if stud_id is not None:
        course_date_id: int = add_course_session(session, course_id, date)
        insert_query = \
        f"""
        ...
        try:
            result = session.execute(insert_query)
            session.commit()
            # logger.info(f"{stud_label}:{stud_id} has been checked at {course_id} at {date}")
            return True
        except sqlalchemy.exc.IntegrityError as ie:
            # logger.error(f'{ie}')
            # logger.info(f"{stud_label}:{stud_id} has already been checked at {course_id} at {date}")
            return False
        finally:
...
if __name__ == '__main__':
    ...
    # course_date_id: int = add_course_session(session, course_id=1, date='2019-12-13')
    # logger.info(course_date_id)

    label = 'aslan'
    course_id = 1
    date = str(datetime.utcnow().date())
    s = check_student_at_course_session(session=session, stud_label=label, course_id=course_id, date=date)
    logger.info(s)
